...
**v1.2.1 — Migration CI Node 22/24.** Retrait de Node 20 (EOL avril 2026, runtime setup-node retiré des runners GitHub le 2026-06-02). Matrice CI `["22", "24"]`, build/déploiement fixés sur Node 22 (LTS active), engine `node` relevé `>=20.19 → >=22.12` sur la racine et les deux packages, `.nvmrc` à `22`. Corepack + pnpm 9 inchangés (montée pnpm 10 = chantier distinct). Aucun changement fonctionnel : détecteurs, formats, exports et API publique strictement identiques à `v1.2.0`. Cf. `docs/adr/0010-bump-node-22-24.md`.
