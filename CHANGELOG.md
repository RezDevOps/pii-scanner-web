...
- **Clarification du discours souveraineté.** L'activation du worker pool app-side rend visibles dans l'onglet Network du navigateur des chargements de chunks JavaScript lors d'un scan : le script du Web Worker au premier scan, et les chunks des parseurs binaires lazy-loadés (`pdfjs-dist`, `mammoth`, `@e965/xlsx`, depuis `v0.4.1` cf. ADR-0007) au premier scan d'un fichier binaire. Toutes ces requêtes pointent vers l'origine de l'app (jamais vers un domaine tiers) et sont régies par les directives CSP `script-src 'self'` / `worker-src 'self' blob:`. La garantie technique de souveraineté est **`connect-src 'none'`** qui interdit au runtime tout `fetch`/`XHR`/`WebSocket`/`EventSource`/`navigator.sendBeacon` — aucune donnée utilisateur ne peut sortir, par construction. Le discours v1.0/v1.1 « aucune requête sortante » était imprécis dès `v0.4.1` (lazy-loading) ; il est reformulé à travers tout le repo en `v1.2.0` : `README.md`, `apps/pii-scanner-web/README.md`, `docs/comment-verifier-souverainete.md`, `docs/exports.md`, `apps/pii-scanner-web/public/verifier/index.html`, `apps/pii-scanner-web/src/app/app.component.ts` (landing + section Souveraineté), `apps/pii-scanner-web/src/app/icons/icon-registry.ts`, `apps/pii-scanner-web/src/index.dev.html`. La promesse de fond reste tenue à l'identique : aucune donnée utilisateur ne quitte le navigateur.
